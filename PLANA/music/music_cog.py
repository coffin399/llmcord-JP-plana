...
try:
    ...
    Track = None; extract_audio_data = None; ensure_stream = None; MusicCogExceptionHandler = None
...
class LoopMode(Enum):
    OFF = auto(); ONE = auto(); ALL = auto()

class GuildState:
    def __init__(self, bot: commands.Bot, guild_id: int, cog_config: dict):
        self.bot = bot; self.guild_id = guild_id; self.voice_client: Optional[discord.VoiceClient] = None
        self.current_track: Optional[Track] = None; self.queue: asyncio.Queue[Track] = asyncio.Queue()
        self.volume: float = cog_config.get('music', {}).get('default_volume', 50) / 100.0
        self.loop_mode: LoopMode = LoopMode.OFF; self.is_playing: bool = False; self.is_paused: bool = False
        self.auto_leave_task: Optional[asyncio.Task] = None; self.last_text_channel_id: Optional[int] = None
        self.connection_lock = asyncio.Lock(); self.last_activity = datetime.now(); self.cleanup_in_progress = False
    def update_activity(self): self.last_activity = datetime.now()
    def update_last_text_channel(self, channel_id: int): self.last_text_channel_id = channel_id; self.update_activity()
        ...
        try:
            if self.voice_client:
                try:
                    ...
                    if self.voice_client.is_connected(): await asyncio.wait_for(self.voice_client.disconnect(force=True), timeout=5.0)
                except Exception as e:
                    guild = self.bot.get_guild(self.guild_id)
                    logger.warning(f"Guild {self.guild_id} ({guild.name if guild else ''}): Voice cleanup error: {e}")
                finally: self.voice_client = None
        finally: self.cleanup_in_progress = False

class MusicCog(commands.Cog, name="music_cog"):
    def __init__(self, bot: commands.Bot):
        self.bot = bot
        if not all((Track, extract_audio_data, ensure_stream, MusicCogExceptionHandler)): raise commands.ExtensionFailed(self.qualified_name, "必須コンポーネントのインポート失敗")
        self.config = self._load_bot_config(); self.music_config = self.config.get('music', {}); self.guild_states: Dict[int, GuildState] = {}
        ...
        self.global_connection_lock = asyncio.Lock(); self.cleanup_task = None
    ...
    def _load_bot_config(self) -> dict:
        ...
        try:
            with open('config.yaml', 'r', encoding='utf-8') as f:
                loaded_config = yaml.safe_load(f); self.bot.config = loaded_config; return loaded_config
    ...
    def cog_unload(self):
        ...
        for guild_id in list(self.guild_states.keys()):
            try:
                ...
                if state.voice_client and state.voice_client.is_connected(): asyncio.create_task(state.voice_client.disconnect(force=True))
        ...
        self.guild_states.clear(); logger.info("MusicCog unloaded.")
    ...
    async def cleanup_task_loop(self):
        try:
            ...
            guilds_to_cleanup = [gid for gid, state in self.guild_states.items() if (current_time - state.last_activity > inactive_threshold and not state.is_playing and (not state.voice_client or not state.voice_client.is_connected()))]
        ...
        except Exception as e: logger.error(f"Cleanup task error: {e}", exc_info=True)
        ...
        if guild_id not in self.guild_states:
            if len(self.guild_states) >= self.max_guilds:
                ...
                if oldest_guild:
                    ...
                    logger.info(f"Removed oldest inactive guild {oldest_guild} ({guild.name if guild else ''}) to make room")
            self.guild_states[guild_id] = GuildState(self.bot, guild_id, self.config)
        self.guild_states[guild_id].update_activity(); return self.guild_states[guild_id]
    async def _send_response(self, interaction: discord.Interaction, message_key: str, ephemeral: bool = False, **kwargs):
        ...
        try:
            ...
            except Exception as e: logger.error(f"Guild {interaction.guild.id} ({interaction.guild.name}): Followup error: {e}")
        except Exception as e: logger.error(f"Guild {interaction.guild.id} ({interaction.guild.name}): Response error: {e}")
    async def _send_background_message(self, channel_id: int, message_key: str, **kwargs):
        try:
            channel = self.bot.get_channel(channel_id)
            if isinstance(channel, discord.TextChannel): await channel.send(self.exception_handler.get_message(message_key, **kwargs))
        except discord.Forbidden: logger.debug(f"No permission to send to channel {channel_id}")
        except Exception as e: logger.error(f"Background message error: {e}")
    ...
    async def _ensure_voice(self, interaction: discord.Interaction, connect_if_not_in: bool = True) -> Optional[discord.VoiceClient]:
        ...
        if not user_voice or not user_voice.channel: await self._send_response(interaction, "join_voice_channel_first", ephemeral=True); return None
        async with state.connection_lock:
            async with self.global_connection_lock:
                active_connections = sum(1 for s in self.guild_states.values() if s.voice_client and s.voice_client.is_connected())
                if active_connections >= self.max_guilds and not state.voice_client: await self._send_response(interaction, "error_playing", ephemeral=True, error="現在接続数が上限に達しています。"); return None
            vc = state.voice_client
            if vc:
                if not vc.is_connected(): await state.cleanup_voice_client(); vc = None
                elif vc.channel == user_voice.channel: return vc
                else: await state.cleanup_voice_client(); await asyncio.sleep(0.5); vc = None
            for voice_client in list(self.bot.voice_clients):
                if voice_client.guild.id == interaction.guild.id and voice_client != state.voice_client:
                    try: await asyncio.wait_for(voice_client.disconnect(force=True), timeout=3.0)
                    except: pass
            if not vc and connect_if_not_in:
                try:
                    await asyncio.sleep(0.3)
                    state.voice_client = await asyncio.wait_for(user_voice.channel.connect(timeout=30.0, reconnect=True, self_deaf=True), timeout=35.0)
                    logger.info(f"Guild {interaction.guild.id} ({interaction.guild.name}): Connected to {user_voice.channel.name}")
                    return state.voice_client
                except Exception as e: await self._handle_error(interaction, e); state.voice_client = None; return None
            elif not vc: await self._send_response(interaction, "bot_not_in_voice_channel", ephemeral=True); return None
            return vc
    async def _play_next_song(self, guild_id: int):
        ...
        if state.loop_mode == LoopMode.ONE and state.current_track: track_to_play = state.current_track
            ...
            try: track_to_play = await state.queue.get(); state.queue.task_done()
            except: pass
        if not track_to_play:
            state.current_track = None; state.is_playing = False
            guild = self.bot.get_guild(guild_id)
            ...
            await state.cleanup_voice_client(); return
        state.current_track = track_to_play; state.is_playing = True; state.is_paused = False; state.update_activity()
        try:
            if not track_to_play.stream_url or not Path(track_to_play.stream_url).is_file():
                ...
                if not (updated_track and updated_track.stream_url): raise RuntimeError("ストリームURLの取得/更新に失敗しました。")
                track_to_play.stream_url = updated_track.stream_url
            source = discord.PCMVolumeTransformer(discord.FFmpegPCMAudio(track_to_play.stream_url, executable=self.ffmpeg_path, before_options=self.ffmpeg_before_options, options=self.ffmpeg_options), volume=state.volume)
            ...
            if state.last_text_channel_id: await self._send_background_message(state.last_text_channel_id, "error_message_wrapper", error=error_message)
            if state.loop_mode == LoopMode.ALL and track_to_play: await state.queue.put(track_to_play)
            state.current_track = None; asyncio.create_task(self._play_next_song(guild_id))
    def _song_finished_callback(self, error: Optional[Exception], guild_id: int):
        state = self._get_guild_state(guild_id)
        if not state: return
        finished_track = state.current_track; state.is_playing = False; state.current_track = None
        if error:
            guild = self.bot.get_guild(guild_id)
            error_message = self.exception_handler.handle_error(error, guild)
            if state.last_text_channel_id: asyncio.run_coroutine_threadsafe(self._send_background_message(state.last_text_channel_id, "error_message_wrapper", error=error_message), self.bot.loop)
    ...
    def _schedule_auto_leave(self, guild_id: int):
        ...
        if not state: return
        if state.auto_leave_task and not state.auto_leave_task.done(): state.auto_leave_task.cancel()
        if state.voice_client and state.voice_client.is_connected(): state.auto_leave_task = asyncio.create_task(self._auto_leave_coroutine(guild_id))
    async def _auto_leave_coroutine(self, guild_id: int):
        await asyncio.sleep(self.auto_leave_timeout)
        state = self._get_guild_state(guild_id)
        if state and state.voice_client and state.voice_client.is_connected() and not [m for m in state.voice_client.channel.members if not m.bot]:
    ...
    async def on_ready(self): logger.info(f"{self.bot.user.name} の MusicCog が正常にロードされました。")
    @commands.Cog.listener()
    async def on_voice_state_update(self, member: discord.Member, before: discord.VoiceState, after: discord.VoiceState):
        if member.id == self.bot.user.id and before.channel and not after.channel: await self._cleanup_guild_state(member.guild.id); return
    ...
    async def play_slash(self, interaction: discord.Interaction, query: str):
        ...
        except Exception as e: await self._handle_error(interaction, e); return
        ...
        for track in tracks:
            if state.queue.qsize() < self.max_queue_size:
                track.requester_id = interaction.user.id; await state.queue.put(track)
    ...
    async def pause_slash(self, interaction: discord.Interaction):
        ...
        state.voice_client.pause(); state.is_paused = True; await self._send_response(interaction, "playback_paused")
    ...
    async def resume_slash(self, interaction: discord.Interaction):
        ...
        state.voice_client.resume(); state.is_paused = False; await self._send_response(interaction, "playback_resumed")
    ...
    async def skip_slash(self, interaction: discord.Interaction):
        ...
        await self._send_response(interaction, "skipped_song", title=state.current_track.title); state.voice_client.stop()
    ...
    async def stop_slash(self, interaction: discord.Interaction):
        ...
        state.loop_mode = LoopMode.OFF; await state.clear_queue()
        if state.voice_client and state.voice_client.is_playing(): state.voice_client.stop()
        state.is_playing = False; state.is_paused = False; state.current_track = None
    ...
    async def leave_slash(self, interaction: discord.Interaction):
        ...
        async with state.connection_lock:
            ...
            await self._send_response(interaction, "leaving_voice_channel"); await state.cleanup_voice_client()
    @app_commands.command(name="queue", description="現在の再生キューを表示します。")
    async def queue_slash(self, interaction: discord.Interaction):
        state = self._get_guild_state(interaction.guild.id)
        if not state: await interaction.response.send_message("エラーが発生しました。", ephemeral=True); return
        state.update_last_text_channel(interaction.channel.id)
        if state.queue.empty() and not state.current_track: await interaction.response.send_message(self.exception_handler.get_message("queue_empty"), ephemeral=True); return
        ...
        async def get_page_embed(page_num: int):
            embed = discord.Embed(title=self.exception_handler.get_message("queue_title", count=total_items + (1 if state.current_track else 0)), color=discord.Color.blue())
            lines = []
            if page_num == 1 and state.current_track:
                ...
                lines.append(f"**{status_icon} {track.title}** (`{format_duration(track.duration)}`) - Req: **{requester.display_name if requester else '不明'}**\n")
        ...
        while True:
            try:
                ...
                new_page = 1 if emoji == "⏪" else max(1, current_page - 1) if emoji == "◀️" else min(total_pages, current_page + 1) if emoji == "▶️" else total_pages if emoji == "⏩" else None
                if emoji == "⏹️": await message.clear_reactions(); return
                if new_page is not None and new_page != current_page: current_page = new_page; await message.edit(embed=await get_page_embed(current_page))
    ...
    async def nowplaying_slash(self, interaction: discord.Interaction):
        state = self._get_guild_state(interaction.guild.id)
        if not state or not state.current_track: await interaction.response.send_message(self.exception_handler.get_message("now_playing_nothing"), ephemeral=True); return
    ...
    async def clear_slash(self, interaction: discord.Interaction):
        state = self._get_guild_state(interaction.guild.id)
        if not state or not await self._ensure_voice(interaction, connect_if_not_in=False): return
        await state.clear_queue(); await self._send_response(interaction, "queue_cleared")
    ...
    async def remove_slash(self, interaction: discord.Interaction, index: app_commands.Range[int, 1, None]):
        state = self._get_guild_state(interaction.guild.id)
        if not state: await interaction.response.send_message("エラーが発生しました。", ephemeral=True); return
        if state.queue.empty(): await interaction.response.send_message(self.exception_handler.get_message("queue_empty"), ephemeral=True); return
    ...
    async def volume_slash(self, interaction: discord.Interaction, level: app_commands.Range[int, 0, 200]):
        state = self._get_guild_state(interaction.guild.id)
        if not state: await interaction.response.send_message("エラーが発生しました。", ephemeral=True); return
        state.volume = level / 100.0; state.update_activity()
    ...
    async def loop_slash(self, interaction: discord.Interaction, mode: app_commands.Choice[str]):
        ...
        state.loop_mode = mode_map.get(mode.value, LoopMode.OFF); state.update_activity()
    ...
    async def join_slash(self, interaction: discord.Interaction):
        ...
        if await self._ensure_voice(interaction, connect_if_not_in=True): await interaction.followup.send(self.exception_handler.get_message("already_connected"), ephemeral=True)
    ...
    async def music_help_slash(self, interaction: discord.Interaction):
        ...
        command_info = {
            "▶️ 再生コントロール / Playback Control": [{"name": "play", "args": "<song name or URL>", "desc_ja": "曲を再生/キュー追加", "desc_en": "Play/add a song"}, {"name": "pause", "args": "", "desc_ja": "一時停止", "desc_en": "Pause"}, {"name": "resume", "args": "", "desc_ja": "再生再開", "desc_en": "Resume"}, {"name": "stop", "args": "", "desc_ja": "再生停止＆キュークリア", "desc_en": "Stop & clear queue"}, {"name": "skip", "args": "", "desc_ja": "現在の曲をスキップ", "desc_en": "Skip song"}, {"name": "volume", "args": "<level 0-200>", "desc_ja": "音量変更", "desc_en": "Change volume"}],
            "💿 キュー管理 / Queue Management": [{"name": "queue", "args": "", "desc_ja": "キュー表示", "desc_en": "Display queue"}, {"name": "nowplaying", "args": "", "desc_ja": "現在再生中の曲", "desc_en": "Show current song"}, {"name": "shuffle", "args": "", "desc_ja": "キューをシャッフル", "desc_en": "Shuffle queue"}, {"name": "clear", "args": "", "desc_ja": "キューをクリア", "desc_en": "Clear queue"}, {"name": "remove", "args": "<queue number>", "desc_ja": "指定番号の曲を削除", "desc_en": "Remove song"}, {"name": "loop", "args": "<off|one|all>", "desc_ja": "ループモード設定", "desc_en": "Set loop mode"}],
            "🔊 ボイスチャンネル / Voice Channel": [{"name": "join", "args": "", "desc_ja": "VCに接続", "desc_en": "Join VC"}, {"name": "leave", "args": "", "desc_en": "Leave VC", "desc_ja": "VCから切断"}]
        }
        cog_command_names = {cmd.name for cmd in self.__cog_app_commands__}
        for category, commands_in_category in command_info.items():
            field_value = "".join(f"`/{c['name']}{' ' + c['args'] if c['args'] else ''}`\n{c['desc_ja']} / {c['desc_en']}\n" for c in commands_in_category if c['name'] in cog_command_names)
            if field_value: embed.add_field(name=f"**{category}**", value=field_value, inline=False)
        active_guilds = len(self.guild_states); embed.set_footer(text=f"<> は引数を表します | Active: {active_guilds}/{self.max_guilds} servers")
        await interaction.followup.send(embed=embed)

async def setup(bot: commands.Bot):
    try: await bot.add_cog(MusicCog(bot)); logger.info("MusicCog successfully loaded")
