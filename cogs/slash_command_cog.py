...
class SlashCommandsCog(commands.Cog, name="スラッシュコマンド"):
    ...
    async def diceroll(self, interaction: discord.Interaction, min_value: int, max_value: int):
        ...
        logger.info(f"/diceroll が実行されました。 (User: {interaction.user.id}, Range: {min_value}-{max_value}, Result: {result})")

    @app_commands.command(name="roll",
                          description="nDn形式でダイスを振ります (例: 2d6+3)。/ Rolls dice in nDn format (e.g., 2d6+3).")
    @app_commands.describe(expression="ダイスの表記 (例: 1d100, 2d6+5, 3d8-2) / Dice notation (e.g., 1d100, 2d6+5, 3d8-2)")
    async def roll(self, interaction: discord.Interaction, expression: str):
        ...
        embed = discord.Embed(title="🎲 ダイスロール結果 / Dice Roll Result", description=f"{interaction.user.mention} がダイスを振りました！", color=discord.Color.purple())
        input_expression = f"{dice_count}d{dice_sides}"
        if modifier > 0: input_expression += f" + {modifier}"
        elif modifier < 0: input_expression += f" - {abs(modifier)}"
        ...
        if modifier != 0 or dice_count > 1:
            details = f" (合計: {total}"
            if modifier > 0: details += f" + {modifier}"
            elif modifier < 0: details += f" - {abs(modifier)}"
            details += ")"
            result_str += details
        embed.add_field(name="最終結果 / Final Result", value=result_str, inline=False)
        embed.timestamp = datetime.datetime.now(datetime.timezone.utc)
        await interaction.response.send_message(embed=embed)
        logger.info(f"/roll が実行されました。 (User: {interaction.user.id}, Expression: {expression}, Result: {final_result})")

    @app_commands.command(name="check",
                          description="ダイスロールと、任意で条件判定を行います。/ Rolls dice and optionally performs a check.")
    @app_commands.describe(expression="ダイスの表記 (例: 1d100, 2d6+5) / Dice notation (e.g., 1d100, 2d6+5)", condition="[任意] 比較条件 / [Optional] Comparison condition", target="[任意] 目標値 / [Optional] Target number")
    ...
    async def check(self, interaction: discord.Interaction, expression: str, condition: Optional[str] = None, target: Optional[int] = None):
        ...
        if is_check:
            ...
            if condition == "<": success = final_result < target_val
            elif condition == "<=": success = final_result <= target_val
            elif condition == ">": success = final_result > target_val
            elif condition == ">=": success = final_result >= target_val
            elif condition == "==": success = final_result == target_val
            ...
            embed = discord.Embed(title=f"{status_emoji} 判定ロール結果 / Check Roll Result", description=f"{interaction.user.mention} が判定を行いました！", color=embed_color)
            dice_expression = f"{dice_count}d{dice_sides}"
            if modifier > 0: dice_expression += f"+{modifier}"
            elif modifier < 0: dice_expression += f"{modifier}"
            rolls_str = ", ".join(map(str, rolls))
    ...
    @app_commands.command(name="ping", description="Botの現在のレイテンシを表示します。/ Shows the bot's current latency.")
    async def ping(self, interaction: discord.Interaction):
        latency_ms = round(self.bot.latency * 1000)
        embed = discord.Embed(title="Pong! 🏓", description=f"現在のレイテンシ / Current Latency: `{latency_ms}ms`", color=discord.Color.green() if latency_ms < 150 else (discord.Color.orange() if latency_ms < 300 else discord.Color.red()))
        await interaction.response.send_message(embed=embed, ephemeral=False)
        logger.info(f"/ping が実行されました。レイテンシ: {latency_ms}ms (User: {interaction.user.id})")

    @app_commands.command(name="serverinfo", description="現在のサーバーに関する情報を表示します。/ Displays information about the current server.")
    async def serverinfo(self, interaction: discord.Interaction):
        if not interaction.guild:
            await interaction.response.send_message("このコマンドはサーバー内でのみ使用できます。\nThis command can only be used within a server.", ephemeral=False)
            return
        ...
        if guild.owner: owner_display = guild.owner.mention
        elif guild.owner_id:
            try:
            ...
            except discord.NotFound: owner_display = f"ID: {guild.owner_id} (取得不可 / Not found)"
    ...
    @app_commands.command(name="userinfo", description="指定されたユーザーの情報を表示します。/ Displays information about the specified user.")
    @app_commands.describe(user="情報を表示するユーザー（任意、デフォルトはコマンド実行者） / User to display information for (optional, defaults to you)")
    async def userinfo(self, interaction: discord.Interaction, user: Optional[discord.User] = None):
        target_user = user or interaction.user
        embed = discord.Embed(title=f"{target_user.display_name} のユーザー情報 / User Information", color=target_user.accent_color or discord.Color.blurple())
        ...
        if interaction.guild and isinstance(target_user, discord.Member):
            ...
            joined_at_text = discord.utils.format_dt(member.joined_at, style='F') if member.joined_at else "不明 / Unknown"
            ...
            embed.add_field(name=f"ロール ({roles_count}) / Roles ({roles_count})", value=roles_display_value, inline=False)
            if member.bot:
                evaluation_lines = ["✅ **認証済みBot** / Verified Bot" if member.public_flags.verified_bot else "❌ **未認証Bot** / Unverified Bot", "👑 **管理者権限** / Administrator Privileges" if member.guild_permissions.administrator else "🔧 **標準権限** / Standard Privileges"]
                embed.add_field(name="Botの評価 / Bot Evaluation", value="\n".join(evaluation_lines), inline=False)
            else:
                if member.joined_at:
                    sorted_members = sorted(interaction.guild.members, key=lambda m: m.joined_at or datetime.datetime.max.replace(tzinfo=datetime.timezone.utc))
                    try:
                ...
                notable_perms_ja = {"管理者": perms.administrator, "サーバー管理": perms.manage_guild, "ロール管理": perms.manage_roles, "追放": perms.kick_members, "BAN": perms.ban_members}
    ...
    @app_commands.command(name="avatar", description="指定されたユーザーのアバター画像URLを表示します。/ Displays the avatar of the specified user.")
    @app_commands.describe(user="アバターを表示するユーザー（任意、デフォルトはコマンド実行者） / User whose avatar to display (optional, defaults to you)")
    async def avatar_command(self, interaction: discord.Interaction, user: Optional[discord.User] = None):
        target_user = user or interaction.user
        avatar_url = target_user.display_avatar.url
        embed = discord.Embed(title=f"{target_user.display_name} のアバター / Avatar", color=target_user.accent_color or discord.Color.default())
    ...
    @app_commands.command(name="arona", description="Arona Music Botのリポジトリを表示します / Shows the Arona Music Bot repository")
    async def arona_repo_slash(self, interaction: discord.Interaction) -> None:
        if self.arona_repository:
            await interaction.response.send_message(f"アロナ (Arona Music Bot) のリポジトリはこちらです！\n{self.arona_repository}\n\nHere is the repository for Arona (Arona Music Bot)!\n{self.arona_repository}", ephemeral=False)
            logger.info(f"/arona が実行されました。 (User: {interaction.user.id})")
        else:
            await interaction.response.send_message("Arona Music BotのリポジトリURLが設定されていません。\nThe repository URL for Arona Music Bot is not set.", ephemeral=False)
            logger.warning(f"/arona が実行されましたが、リポジトリURL未設定。 (User: {interaction.user.id})")

    @app_commands.command(name="plana", description="llmcord-JP-planaのリポジトリを表示します / Shows the llmcord-JP-plana repository")
    async def plana_repo_slash(self, interaction: discord.Interaction) -> None:
        if self.plana_repository:
            await interaction.response.send_message(f"プラナ (llmcord-JP-plana) のリポジトリはこちらです！\n{self.plana_repository}\n\nHere is the repository for Plana (llmcord-JP-plana)!\n{self.plana_repository}", ephemeral=False)
            logger.info(f"/plana が実行されました。 (User: {interaction.user.id})")
        else:
            await interaction.response.send_message("llmcord-JP-planaのリポジトリURLが設定されていません。\nThe repository URL for llmcord-JP-plana is not set.", ephemeral=False)
            logger.warning(f"/plana が実行されましたが、リポジトリURL未設定。 (User: {interaction.user.id})")

    @app_commands.command(name="support", description="開発者へのお問い合わせ方法を表示します / Shows how to contact the developer")
    async def support_contact_slash(self, interaction: discord.Interaction) -> None:
        embed = discord.Embed(title="💬 お問い合わせ / Contact Support", description="Botに関するご質問・ご要望・不具合報告などは、以下の方法でお気軽にお問い合わせください。\nFor questions, requests, or bug reports about the bot, please feel free to contact us using the methods below.", color=discord.Color.blue())
        embed.add_field(name="🐦 X (Twitter)", value=f"DMまたはメンションでお問い合わせください。\nPlease contact via DM or mention.\n[**@coffin299**]({self.support_x_url})", inline=False)
        embed.add_field(name="💬 Discord", value=f"DiscordのDMでお問い合わせください。\nPlease contact via Discord DM.\n**ユーザー名 / Username:** `{self.support_discord_id}`", inline=False)
        embed.add_field(name="📝 ご連絡時のお願い / When Contacting", value="• Botを使用しているサーバー名をお知らせください。\n• 具体的な問題や要望をお書きください。\n• スクリーンショットがあれば添付してください。\n\n• Please mention the server name where you're using the bot.\n• Describe the specific issue or request.\n• Attach screenshots if available.", inline=False)
        embed.set_footer(text="お気軽にお問い合わせください！ / Feel free to contact us!")
        view = discord.ui.View()
        view.add_item(discord.ui.Button(label="X (Twitter)で連絡 / Contact on X", style=discord.ButtonStyle.link, url=self.support_x_url, emoji="🐦"))
        await interaction.response.send_message(embed=embed, view=view, ephemeral=False)
        logger.info(f"/support が実行されました。 (User: {interaction.user.id})")

    @app_commands.command(name="invite", description="このBotをあなたのサーバーに招待します。/ Invites this bot to your server.")
    async def invite_bot_slash(self, interaction: discord.Interaction) -> None:
        ...
        if invite_url_to_display and invite_url_to_display not in ["YOUR_BOT_INVITE_LINK_HERE", "HOGE_FUGA_PIYO"]:
            embed = discord.Embed(title=f"{bot_name} をサーバーに招待 / Invite {bot_name} to Your Server", description=f"下のボタンからPLANAをあなたのサーバーに招待できます！\n\nYou can invite PLANA to your server using the button below!", color=discord.Color.og_blurple())
            ...
            view = discord.ui.View()
            view.add_item(discord.ui.Button(label="サーバーに招待 / Invite to Server", style=discord.ButtonStyle.link, url=invite_url_to_display, emoji="💌"))
            await interaction.response.send_message(embed=embed, view=view, ephemeral=False)
